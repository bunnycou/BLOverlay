function load() {
    ...
    let static = localStorage.getItem("static")
    ...
    // if (!game) { game = "Game 1" } else { game = "Game " + game } // game is originally an integer, red score + blue score
    if (!game) { game = 1 }
    if (!patch) { patch = "Patch " + getLatest() } else { patch = "Patch " + patch }
    if (!league) { league = "Stout" }
    if (!static) { static = "" }
    if (!motd) { motd = "" }
